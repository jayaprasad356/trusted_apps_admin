        ...
        if (isset($_GET['table']) && $_GET['table'] == 'users') {
            ...
            if (isset($_GET['order']))
                ...
                if (isset($_GET['search']) && !empty($_GET['search'])) {
                    ...
                    $where .= " AND (id LIKE '%" . $search . "%' OR name LIKE '%" . $search . "%' OR refer_code LIKE '%" . $search . "%' OR email LIKE '%" . $search . "%')";
                }
            if (isset($_GET['sort'])){
                $sort = $db->escapeString($_GET['sort']);
            }
...
//Share Apps
if (isset($_GET['table']) && $_GET['table'] == 'share_apps') {
    ...
    if (isset($_GET['order']))
        ...
        if (isset($_GET['search']) && !empty($_GET['search'])) {
            $search = $db->escapeString($_GET['search']);
            $where .= " AND (l.id LIKE '%" . $search . "%' OR u.name LIKE '%" . $search . "%' OR a.name LIKE '%" . $search . "%')";
        }
        $join = "LEFT JOIN `users` u ON l.user_id = u.id LEFT JOIN `apps` a ON l.app_id = a.id WHERE l.id IS NOT NULL " . $where;

        $sql = "SELECT COUNT(l.id) AS total FROM `share_apps` l " . $join;
        $db->sql($sql);
        $res = $db->getResult();
        foreach ($res as $row) {
            $total = $row['total'];
        ...
        $sql = "SELECT l.id AS id, l.*, u.name AS user_name,a.name AS app_name FROM `share_apps` l " . $join . " ORDER BY $sort $order LIMIT $offset, $limit";
        $db->sql($sql);
        $res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        //$operate = ' <a href="edit-user_plan.php?id=' . $row['id'] . '"><i class="fa fa-edit"></i>Edit</a>';
        $operate = ' <a class="text text-danger" href="delete-share_apps.php?id=' . $row['id'] . '"><i class="fa fa-trash"></i>Delete</a>';
        $tempRow['id'] = $row['id'];
        $tempRow['user_name'] = $row['user_name'];
        $tempRow['app_name'] = $row['app_name'];
        $tempRow['datetime'] = $row['datetime'];
